...
commentController.user = async (req, res, next) => {
    let result;
    if (req.params.id) {
        try {
        ...
        return res.status(400).send("No 'id' parameter provided in endpoint");
};
...
commentController.new = async (req, res, next) => {
    ...
    if (result.rows.length > 0) {
        ...
        return res.status(500).send('DB Error: New comment not returned in query result');
};
...
commentController.put = async (req, res, next) => {
    ...
    if (result.rows.length > 0) {
        ...
        return res.status(500).send('DB Error: Updated comment not returned in query result');
};
