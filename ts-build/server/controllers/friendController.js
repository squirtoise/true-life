...
friendController.all = async (req, res, next) => {
    let queryResult;
    try {
    ...
    if (queryResult.rows.length > 0) {
        ...
        return res.status(404).send('No friends found under specified user ID');
};
...
friendController.requests = async (req, res, next) => {
    const query = req.query.reqs === 'sent'
    ...
    if (query) {
        ...
        if (queryResult.rows.length > 0) {
            ...
            return res.status(404).send(`No ${req.query.reqs}`);
    }
    else
        return res
            .status(400)
            .send(`Endpoint requires 'req' query to either equal 'sent' (for sent friend reqs) or 'received' (for received friend reqs)`);
};
...
friendController.new = async (req, res, next) => {
    ...
    const requestCheck = await db.query(queries.getFriendReq, [req.body.friend, req.params.id]);
    ...
    if (requestCheck?.rows && requestCheck.rows.length > 0) {
        ...
        const update = await db.query(queries.addFriendByReq, [req.body.friend, req.params.id]);
        // if update succeeds
        if (update?.rows?.length > 0) {
            // add another row to make the friendship mutual
            const add = await db.query(queries.addFriend, [req.params.id, req.body.friend]);
            if (add?.rows && add.rows.length > 0) {
                ...
                return res.status(500).send('DB error: Adding second friend relational row failed');
        }
        else
            return res.status(500).send('DB error: Friend request update failed');
    }
    else
        return res.status(404).send('Friend request does not exist');
};
...
friendController.send = async (req, res, next) => {
    let userCheck;
    try {
    ...
    if (userCheck.rows.length > 0) {
        ...
        return res.status(404).send('Friend ID specified in body does not match any user ID in dB.');
};
...
friendController.deny = async (req, res, next) => {
...
friendController.del = async (req, res, next) => {
