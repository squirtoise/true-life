...
postController.user = async (req, res, next) => {
    ...
    if (result.rows.length > 0) {
        ...
        res.status(404).send('No user posts found in DB, either user ID is wrong or user has no posts');
};
// TEST THIS
...
postController.friends = async (req, res, next) => {
    ...
    if (result.rows.length > 0) {
        ...
        return res
            .status(404)
            .send(`No friend posts found in DB, either user ID is wrong, user has no friends, or user's friend's have no posts`);
};
...
postController.one = async (req, res, next) => {
    ...
    if (result.rows.length > 0) {
        ...
        return res.status(404).send('Post not found in DB, double check post ID provided is correct');
};
...
postController.new = async (req, res, next) => {
    ...
    if (req.file) {
        ...
        return res.status(400).send('File (req.file) required in post upload');
};
