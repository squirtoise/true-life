...
router.get('/comment/user/:id', commentController.user, (req, res, next) => {
    return res.json(res.locals.comments);
});
// returns array of all a post's comments
// req.params.id : ID of post whose comments are requested
router.get('/comment/:id', commentController.all, (req, res, next) => {
    return res.json(res.locals.comments);
});
...
router.post('/:id', upload.single('image'), postController.new, s3Controller.put, (req, res, next) => {
    return res.json(res.locals.post);
});
...
router.post('/comment/:id', commentController.new, (req, res, next) => {
    return res.json(res.locals.comment);
});
// updates a post and returns updated post
// req.params.id : ID of post to be updated
router.put('/:id', postController.put, (req, res, next) => {
    return res.json(res.locals.post);
});
// updates a comment on a post
// req.params.id : ID of comment to be updated
router.put('/comment/:id', commentController.put, (req, res, next) => {
    return res.json(res.locals.comment);
});
// deletes a post
// req.params.id : ID of post to be deleted
router.delete('/:id', postController.del, (req, res, next) => {
    return res.send('Post deleted');
});
// deletes a comment
// req.params.id : ID of comment to be deleted
router.delete('/comment/:id', commentController.del, (req, res, next) => {
